...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        policy: ActorCritic | ActorCriticRecurrent | ActorCriticPointNet | StudentTeacher | StudentTeacherRecurrent | StudentTeacherPointNet = policy_class(
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if locs["ep_infos"]:
            num_envs = sum([x.get("num_envs",0) for x in locs["ep_infos"]])
            ...
            mean_envs = torch.mean(torch.tensor([x.get("num_envs",0.0) for x in locs["ep_infos"]]))
            if mean_envs != 0.0:
